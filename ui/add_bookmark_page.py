...
def render_add_bookmark_page(db, vector_store, categorize_agent, debug):
    ...
    if 'insta_logged_in' not in st.session_state or not st.session_state["insta_logged_in"]:
        ...
        st.button("로그인 페이지로 이동", on_click=change_menu, args=("랜딩 페이지",))
    
    # 북마크가 로드되었는지 확인
    if 'fetched_bookmarks' not in st.session_state:
        st.warning("먼저 북마크를 불러와야 합니다.")
        st.button("북마크 불러오기 페이지로 이동", on_click=change_menu, args=("랜딩 페이지",))
    ...
    with st.expander("DB 및 Vector Store 저장 현황"):
        st.write(f"""
        {"✅" if 'db_saved' in st.session_state and st.session_state["db_saved"] else "❌"}      DB\n
        {"✅" if 'vector_saved' in st.session_state and st.session_state["vector_saved"] else "❌"}  Vector Store
        """)
    ...
    if ('fetched_bookmarks' in st.session_state and 
        ...
        #         del st.session_state.search_output
            
        #     st.session_state.current_menu = "랜딩 페이지"
        #     st.rerun()
        st.button("처음으로 돌아가기", on_click=initialize)